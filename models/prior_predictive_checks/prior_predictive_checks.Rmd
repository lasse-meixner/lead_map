...
For now, I take no additional preprocessing steps on the predictors passed to the logit side, income, building period, and SVI.
...
```
```{r}
# want to extract the samples
ppc_samples$summary() |> filter(str_detect(variable, "y_thinned")) |> knitr::kable(digits = 3)
```
