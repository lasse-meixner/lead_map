...
model {
  ...
  gamma ~ normal(0, 1.5);
  // model
  vector[N_obs] mu_obs = exp(alpha + beta * x_obs + omega_p * w_obs);
  vector[N_obs] pi_obs = inv_logit(gamma + delta * z_obs + kappa * x_obs + omega_l * w_obs);
  vector[N_obs] lambda_obs = mu_obs .* pi_obs; // elementwise product
  y_obs ~ poisson(lambda_obs); 
  for (j in 1:N_cens) {
    real mu_cens = exp(alpha + beta * x_cens[j] + omega_p * w_cens[j]);
    real pi_cens = inv_logit(gamma + delta * z_cens[j] + kappa * x_cens[j] + omega_l * w_cens[j]);
