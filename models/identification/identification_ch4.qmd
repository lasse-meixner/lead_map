...
ma_sample_prior <- sample_prior(ma_final, 100, c(1,1,1))
ma_sample_prior |>
...
```{r}
# sample from the prior
ma_sample_prior <- sample_prior(ma_final, 100, c(3,3,3))
ma_sample_prior |>
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") |>
...
```{r}
# sample from the prior
ma_sample_prior <- sample_prior(ma_final, 100, c(10,10,10))
ma_sample_prior |>
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") |>
  ggplot(aes(x = value, color = variable, alpha = 0.2)) +
  geom_density() +
