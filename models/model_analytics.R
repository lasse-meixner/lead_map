...
single_state_tract <- function(state_name, drop_outcome = c(), info_vars = tract_info_vars, pred_preprocess_func = NULL){
...
single_state_zip <- function(state_name, drop_outcome = c(), info_vars = zip_info_vars, pred_preprocess_func = NULL){
    # load and assign the state data
    ...
    state_pred <- zip_data |> filter(STATE_ABBR == state_name) |> preprocess_pred_data(info_vars = info_vars, additional_preprocess = pred_preprocess_func)
