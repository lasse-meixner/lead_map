...
The Rhat values are not perfect but not too bad for the coefficients on the Poisson side, from the plot of posteriors we can see that they are nicely shaped. As expected the intercepts alpha and gamma are not identified at all.

What happens if we nail down the prior on alpha?

```{r}
stan_model_w_priors <- cmdstan_model("poisson_thinned_exclusion_w_priors.stan")

# append nhanes_prior_var to the data
stan_data <- stan_data |>
    append(list(nhanes_prior_var = 1))

fit_w_priors <- stan_model_w_priors$sample(
  data = stan_data,seed = 1234,
  chains = 4, 
  parallel_chains = 4,
  refresh = 500)

fit_w_priors |> get_coefficients() |> knitr::kable(digits = 3)
```

Setting the variance of alpha to 1 helps! This looks much better. But if you narrow down the prior even further, things actually look worse:

```{r}
# set prior variance to 0.01
stan_data[["nhanes_prior_var"]] <- 0.01

# sample again
fit_w_priors_2 <- stan_model_w_priors$sample(
  data = stan_data,seed = 1234,
  chains = 4, 
  parallel_chains = 4,
  refresh = 500)

fit_w_priors_2 |> get_coefficients() |> knitr::kable(digits = 3)
```

This is worse than before...

Let us now look at including some features in the logit side.
...
Unsurprisingly, throwing all of the x's into the logit side, this does not look good at all. Inspecting the intercorrelation of these features:
...
The logit coefficients are spread flat, so not located at all. Interestingly, it seems to be especially the variables included in the logit that are badly identified. **(I checked and this pattern also appeared when including two X's on the logit side for tract data...)** I retry the same with including the house price and the svi_socioeconomic_pctile.
...
This looks slightly better... But convergence statistics are still pretty bad.
...
This pattern did not appear in the simple simulation where our data is nicely spread and normal. So I suspect this pattern has to do with particularities of our DGP. Looking at the distribution of the pi's from the generated quantities
...
It looks like most of these values are stuck to 1.
...
The issue here does not appear to be multicollinearity on the logit side... Some observations:

-   Narrowing down the prior on alpha too much, even without having any x's in the logit can make things worse!

-   Including even uncorrelated x's on the logit side messes up efficient sampling of posteriors. It is unclear, especially because RI is so small, if this is an issue with the model or just too little information in the small RI dataset.

-   Interestingly, it is especially the variables included in the logit that are badly identified on the poisson side. I couldnt yet replicate this pattern in the simulations in the identification branch.

-   Most of the draws for the pi's are stuck at 1. So log-odds are estimated to be high, and the logistic is practically **flat** und approximately **linear** on this set.

I try to get some better understanding of all of these observations back in the simulation setting in 'models/identification/identification_ch2.qmd'.

### Michigan (higher ell threshold)

Michigan has more complicated data, it has suppression in BLLs and tested. However, the body of data is much larger. Let's have a look.
...
```{r}
# prep data
mi_data <- single_state_data("MI")

# manually dropping MI duplicates
print(length(mi_data))
mi_data <- mi_data |> distinct()
print(length(mi_data))
```

```{r}
# build STAN vector
stan_data_mi <- build_stan_vector(mi_data)
```

```{r}
# append nhanes_prior_var to the data
stan_data_mi <- stan_data_mi |>
    append(list(nhanes_prior_var = 1))

fit_mi_w_priors <- stan_model_w_priors$sample(
  data = stan_data_mi,
  seed = 1234,
  chains = 4, 
  parallel_chains = 4,
...
Even without including x's on the logit side, poisson coefficients are not efficiently sampled...
