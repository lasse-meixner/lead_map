...
```{r}
library(bayesplot)
draws <- fit_pca$draws(format = "draws_df") |>
   select(-lp__) |> 
   # rename betas by poisson features names
   rename_with(~poisson_features[as.numeric(gsub("\\D", "", .))] %>% as.character(), starts_with("beta")) |>
   # rename deltas by logit features names with _logit suffix
...
## More precise prior on 𝜶

```{r}
# load the new model
pooled_model_2 <- cmdstan_model("poisson_thinned_exclusion_priors2.stan")

# refit the same data
fit_pca_2 <- pooled_model_2$sample(
  data = stan_data_pca, 
  chains = 4,
  seed = 1999,
  parallel_chains = 4,
...
```{r}
draws <- fit_pca_2$draws(format = "draws_df") |>
   select(-lp__) |> 
   # rename betas by poisson features names
   rename_with(~poisson_features[as.numeric(gsub("\\D", "", .))] %>% as.character(), starts_with("beta")) |>
   # rename deltas by logit features names with _logit suffix
