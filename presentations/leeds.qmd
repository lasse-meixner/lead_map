...
fig.update_layout(title_text="Proportion of population under 5 years old in Leeds")
fig.show()
```
...
plot_histograms("build_year_median", merged, "Leeds", "UK", n_bins=16)
fig.update_layout(title_text="Leeds has older housing relative to the UK")
fig.show()
```

## Location of very old housing stock
```{python}
...
fig.update_layout(title="Proportion of unemployed people in Leeds")
fig.show()
```
...
fig.update_layout(title="Proportion of single parent families in Leeds")
fig.show()
```
...
fig.update_layout(title="Correlation between black population and poverty rate")
fig.show()
```
...
fig.update_layout(title="Distribution of black population in Leeds")
fig.show()
...
fig.update_layout(title="Soil lead concentration in Leeds")
fig.show()
```
